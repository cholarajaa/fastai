...
class WideResNetConcat(nn.Module):
    def __init__(self, num_groups, N, num_classes, k=1, drop_p=0.0, start_nf=16):
        super().__init__()
        n_channels = [start_nf]
        for i in range(num_groups): n_channels.append(start_nf*(2**i)*k)
        ...
        layers += [nn.BatchNorm2d(n_channels[3]), nn.ReLU(inplace=True), AdaptiveConcatPool2d(1),
                   Flatten(), nn.Linear(n_channels[3]*2, num_classes)]
        self.features = nn.Sequential(*layers)

    def forward(self, x): return self.features(x)

def wrn_22(): return WideResNet(num_groups=3, N=3, num_classes=10, k=6, drop_p=0.)
def wrn_22_cat(): return WideResNetConcat(num_groups=3, N=3, num_classes=10, k=6, drop_p=0.)
